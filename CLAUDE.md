...
- **Authentification : Clerk porte l'identité, Jiska garde l'autorisation.** Clerk gère connexion, mot de passe et sessions ; la table `users` (colonne `clerk_id`) reste l'autorité sur qui entre et avec quel rôle. Un compte Clerk sans ligne locale est refusé, et `/login` le dit au lieu de rediriger (sinon : boucle `/app` ↔ `/login`). Le lien se fait par e-mail à la première connexion. Création de compte = ligne locale + invitation Clerk (`/api/users`, dirigeants). Ne jamais dupliquer les rôles côté Clerk : pas d'Organizations. **Aucune interface Clerk encastrée non plus** : photo, prénom, nom et adresse restent gérés par Jiska seule (`/app/profil`), le mot de passe passe par un formulaire maison adossé à `user.updatePassword()`. Deux écrans pour la même donnée finiraient par diverger.
